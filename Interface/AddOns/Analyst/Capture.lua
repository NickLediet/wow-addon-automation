...
(C) Copyright 2007,2016 Bethink (bethink at naef dot com)
...
ANALYST_ACTIVITY_MISSIONS = "MISSIONS"						-- mission detail (string)
...
function Analyst:EnableCapture ()
	...
	self:SecureHook(C_TradeSkillUI, "CraftRecipe")
	...
	self:SecureHook(C_Garrison, "StartMission")
	self:SecureHook(C_Garrison, "MarkMissionComplete")
...
function Analyst:UNIT_SPELLCAST_CHANNEL_START (event, unit, spell)
	-- Don't process foreign events
	if unit ~= "player" or not self.loggedIn or not self.inWorld or not spell then
		return
	end
...
function Analyst:CHAT_MSG_MONEY (event, message)
	if IsInGroup() and (self:MatchFormat(message, LOOT_MONEY_SPLIT)
			or self:MatchFormat(message, LOOT_MONEY_SPLIT_GUILD)) then
		self:SetEvent(
			ANALYST_ACTIVITY_LOOTING,
...
function Analyst:CHAT_MSG_LOOT (event, message)
	...
	if IsInGroup() and (self:MatchFormat(message, LOOT_ITEM_BONUS_ROLL_SELF)
			or self:MatchFormat(message, LOOT_ITEM_BONUS_ROLL_SELF_MULTIPLE)
			or self:MatchFormat(message, LOOT_ITEM_PUSHED_SELF)
			or self:MatchFormat(message, LOOT_ITEM_PUSHED_SELF_MULTIPLE)
			or self:MatchFormat(message, LOOT_ITEM_SELF)
			or self:MatchFormat(message, LOOT_ITEM_SELF_MULTIPLE)) then
		self:SetEvent(
			ANALYST_ACTIVITY_LOOTING,
			self:CreateString(L["GROUPED"]),
			"",
...
function Analyst:CraftRecipe (index)
	local tradeSkillLine = select(2, C_TradeSkillUI.GetTradeSkillLine())
	local recipeInfo = C_TradeSkillUI.GetRecipeInfo(index)
	if tradeSkillLine == self.spells.ALCHEMY then
		self:SetPendingEvent(
			ANALYST_ACTIVITY_ALCHEMY,
			self:CreateEnchant(recipeInfo.recipeID),
			"",
			"",
			ANALYST_COMPLETION_SPELLCAST)
	...
	self.pendingSpell = recipeInfo.name
end
...
function Analyst:AbandonQuest ()
	...
	if string.len(name) > 0 then
		self:SetEvent(
			ANALYST_ACTIVITY_QUESTING,
			self:CreateString(QuestUtils_GetQuestName(questId)),
			"",
			"",
			ANALYST_COMPLETION_TIMED)
	end
...
-- Handles quest turn-in
function Analyst:QUEST_TURNED_IN (event, questId)
	local name = QuestUtils_GetQuestName(questId)
	if string.len(name) > 0 then
		self:SetEvent(
			ANALYST_ACTIVITY_QUESTING,
...
-- Handles quest loot
function Analyst:QUEST_LOOT_RECEIVED (event, questId)
	self:SetEvent(
		ANALYST_ACTIVITY_QUESTING,
		self:CreateString(QuestUtils_GetQuestName(questId)),
		"",
...
function Analyst:SendMail (target, subject, body)
	...
	if itemCount > 0 or GetSendMailMoney() > 0 then
...
-- Handles start of mission
function Analyst:StartMission (missionId)
	self:SetEvent(
		ANALYST_ACTIVITY_MISSIONS,
		self:CreateString(C_Garrison.GetMissionName(missionId)),
		"",
		"",
		ANALYST_COMPLETION_TIMED)
end

-- Handles completion of mission
function Analyst:MarkMissionComplete (missionId)
	self:SetEvent(
		ANALYST_ACTIVITY_MISSIONS,
		self:CreateString(C_Garrison.GetMissionName(missionId)),
		"",
...
function Analyst:CreateLocation ()
	...
	return string.format("%s (%.0f,%.0f)", zoneText, position.x * 100.0, position.y * 100.0)
end
